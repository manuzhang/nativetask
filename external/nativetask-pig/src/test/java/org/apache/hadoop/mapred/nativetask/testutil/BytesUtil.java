...
public class BytesUtil {
  ...
  private static Object newPigObject(byte[] seed, String className) {
    ...
    } else if (className.equals(NullableBigIntegerWritable.class.getName())) {
			pigNullableWritable = new NullableBigIntegerWritable(BigInteger.TEN);
			pigNullableWritable.setNull(false);
		} else if (className.equals(NullableBigDecimalWritable.class.getName())) {
			pigNullableWritable = new NullableBigDecimalWritable(BigDecimal.TEN);
			pigNullableWritable.setNull(false);
		}
  ...
  private static Tuple createPigTuple(byte[] seed) {
    ...
    if (seed.length >= 4) {
      tuple.append(new Integer(BytesUtil.toInt(seed)));
      tuple.append(r.nextFloat());
    }
    if (seed.length >= 8) {
      tuple.append(new Long(BytesUtil.toLong(seed)));
      tuple.append(r.nextDouble());
      tuple.append(new DateTime(BytesUtil.toLong(seed)));
    } 
    return tuple;
  }
