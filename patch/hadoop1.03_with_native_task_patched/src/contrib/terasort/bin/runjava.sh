...
# run bench
$HADOOP_HOME/bin/hadoop jar $HADOOP_HOME/hadoop-examples*.jar terasort \
       -libjars $HADOOP_HOME/lib/hadoop-snappy-0.0.1-SNAPSHOT.jar \
       -D mapred.job.name="terasort_java" \
       -D io.sort.mb=1200 \
       ...
       -D mapred.output.compression.codec=org.apache.hadoop.io.compress.SnappyCodec \
       -D mapred.reduce.tasks=$NUM_REDS $INPUT_HDFS $OUTPUT_JAVA
