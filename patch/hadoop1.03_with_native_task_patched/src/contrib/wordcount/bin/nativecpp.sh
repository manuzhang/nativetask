...
# run bench
$HADOOP_HOME/bin/hadoop jar $HADOOP_HOME/lib/hadoop-nativetask-0.1.0.jar \
    -D mapred.job.name="wordcount_nativecpp" \
    -D io.sort.mb=300 \
    -D mapred.compress.map.output=true \
    ...
    -reader NativeTask.LineRecordReader -writer NativeTask.TextIntRecordWriter -mapper NativeTask.WordCountMapper -reducer NativeTask.IntSumReducer -combiner NativeTask.IntSumReducer -input $INPUT_HDFS -output $OUTPUT_NATIVE 
