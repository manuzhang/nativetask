...
import org.apache.hadoop.mapred.TaskDelegation.MapOutputCollectorDelegator;
...
public class BlockMapOutputBuffer<K extends BytesWritable, V extends BytesWritable>
  ...
  private MemoryBlockAllocator memoryBlockAllocator;
  ...
  private void mergeParts() throws IOException, InterruptedException,
    ...
    for (int i = 0; i < numSpills; i++) {
      filename[i] = task.mapOutputFile.getSpillFile(i);
    ...
    if (numSpills == 1 && !hasInMemorySpill) {
      ...
      if (indexCacheList.size() == 0) {
        rfs.rename(task.mapOutputFile.getSpillIndexFile(0), new Path(
    ...
    for (int i = indexCacheList.size(); i < numSpills; ++i) {
      Path indexFileName = task.mapOutputFile.getSpillIndexFile(i);
    ...
    Path finalOutputFile = task.mapOutputFile.getOutputFileForWrite(
        finalOutFileSize);
    Path finalIndexFile = task.mapOutputFile.getOutputIndexFileForWrite(
        finalIndexFileSize);
  ...
  public void init(TaskUmbilicalProtocol umbilical, TaskReporter reporter,
    ...
    float spillper = job.getFloat("io.sort.spill.percent", (float) 0.8);
