...
implements RawKeyValueIterator, OutputCollector<K, V>, ICombineHandler {
  ...
  public CombinerHandler(Configuration conf, Class<K> iKClass, Class<V> iVClass,
  ...
  public static <K, V> ICombineHandler create(
      Configuration conf, 
      Class<K> iKClass, 
      Class<V> iVClass,
      int inputBufferCapacity,
    ...
    if (null == combinerClazz) {
      ...
      CombinerHandler handler = new CombinerHandler<K, V>(
        conf, 
        iKClass, 
        iVClass, 
        inputBufferCapacity, 
