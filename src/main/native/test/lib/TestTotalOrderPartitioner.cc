...
static void TestRandomTrieSearch(int64_t seed, int64_t size, int64_t partition, uint32_t maxlevel) {
  Random rand(seed);
  string range;
...
//      LOG("[%s]<=[%s]<[%s]", lower.c_str(), key.c_str(), upper.c_str());
  ...
  LOG("Size: %lld Partition: %lld Level: %u Space: %lu Time: %.3lf", (long long int )size,
      (long long int )partition, maxlevel, trie.size(), (timer.now() - timer.last()) / 1000000000.0);
}
