...
TEST(MahoutComparator,StringTupleComparator) {
  ComparatorPtr comparator = NativeTask::get_comparator(StringTupleType, NULL);
...
TEST(MahoutComparator, VarIntComparator) {
  ComparatorPtr comparator = NativeTask::get_comparator(VarIntType, NULL);
  int32_t num_list1[] = {INT_MIN, -1, 0, 1, 100, INT_MAX};
  int32_t num_list2[] = {INT_MIN + 1, 0, 1, 0, 101, INT_MAX};
  int32_t result[] = {-1, -1, -1, 1, -1, 0};
  comparatorTest(num_list1, num_list2, result, sizeof(result) / sizeof(result[0]), comparator,
      &WritableUtils::WriteSignedVarInt);
}

TEST(MahoutComparator, VarLongComparator) {
  ComparatorPtr comparator = NativeTask::get_comparator(VarLongType, NULL);
  int64_t num_list1[] = {-100, -1, 0, 1, 100, 127};
  int64_t num_list2[] = {-101, 0, 1, 0, 101, 127};
  int32_t result[] = {1, -1, -1, 1, -1, 0};
  comparatorTest(num_list1, num_list2, result, sizeof(result) / sizeof(result[0]), comparator,
      &WritableUtils::WriteSignedVarLong);
}

inline int writeGram(char* pos, const char* array, uint32_t length) {
  uint32_t tmp = 0;
  memset(pos, 0, bufferSize);
  WritableUtils::WriteUnsignedVarInt(length, pos, tmp);
  memcpy(pos + tmp, array, length);
  return tmp + length;
}

TEST(MahoutComparator, GramComparator) {
  ComparatorPtr comparator = NativeTask::get_comparator(GramType, NULL);
  char* src = new char[bufferSize];
  char* dest = new char[bufferSize];
  uint32_t srclength;
  uint32_t destlength;
...
inline void writeGramKey(char* pos, char* gram, char* order, uint32_t gramlength,
  ...
  uint32_t ovarint = 0;
  memset(pos, 0, bufferSize);
  WritableUtils::WriteUnsignedVarInt(gramlength + ordersize, pos, gvarint);
  WritableUtils::WriteUnsignedVarInt(gramlength, pos + gvarint, ovarint);
  memcpy(pos + gvarint + ovarint, gram, gramlength);
  memcpy(pos + gvarint + ovarint + gramlength, order, ordersize);
}

TEST(MahoutComparator, GramKeyComparator) {
  ComparatorPtr comparator = NativeTask::get_comparator(GramKeyType, NULL);
  char* src = new char[bufferSize];
  char* dest = new char[bufferSize];
  uint32_t srclength;
  uint32_t destlength;
...
inline void writeSplitPartitioned(char* pos, uint32_t taskId, uint64_t itemOrdinal,
  ...
  uint32_t tmp = 0;
  memset(pos, 0, bufferSize);
  WritableUtils::WriteUnsignedVarInt(taskId, pos, tmp);
  WritableUtils::WriteUnsignedVarLong(itemOrdinal, pos + tmp, length);
  length += tmp;
}

TEST(MahoutComparator, SplitPartitionedComparator) {
  ComparatorPtr comparator = NativeTask::get_comparator(SplitPartitionedType, NULL);
  char* src = new char[bufferSize];
  char* dest = new char[bufferSize];
  uint32_t srclength;
  uint32_t destlength;
...
inline void writeEntityEntity(char* pos, int64_t Id_a, int64_t Id_b, uint32_t &length) {
  uint32_t tmp = 0;
  memset(pos, 0, bufferSize);
  WritableUtils::WriteSignedVarLong(Id_a, pos, tmp);
  WritableUtils::WriteSignedVarLong(Id_b, pos + tmp, length);
  length += tmp;
}

TEST(MahoutComparator, EntityEntityComparator) {
  ComparatorPtr comparator = NativeTask::get_comparator(EntityEntityType, NULL);
  char* src = new char[bufferSize];
  char* dest = new char[bufferSize];
  uint32_t srclength;
  uint32_t destlength;
